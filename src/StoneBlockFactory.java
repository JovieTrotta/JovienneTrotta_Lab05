...
/**
 This class will define stone factory, which implements the Factory interface.
...
public class StoneBlockFactory implements Factory{
    ...
    /**
     ...
     Has an instance of the Resource class named resourceBin, which takes an initial weight (double) (0) and type (ResourceType) (stone).
     */
    ...
    /**
     ...
     If the resourceBin weight (double) reaches or surpasses its limit (20), subtracts 20 from the resourceBin.
     @return new StoneBlock (Block).
     Else, return null.
     */
    @Override
    public Block produce() {
        if (this.resourceBin.getWeight() >= Const.stoneBlockWeight) {
            this.resourceBin.subtractResources(Const.stoneBlockWeight);
            System.out.println("Stone block built!");
    ...
    public void takeResource(Object object) {
        if (object instanceof Resource) {
            switch (((Resource) object).type) {
                case WOOD -> throw new IllegalArgumentException("Stone Block Factory: Can only accept Resource type STONE, not WOOD");
                case STONE -> resourceBin.weight = resourceBin.weight + ((Resource) object).weight;
                case HOUSE -> throw new IllegalArgumentException("Stone Block Factory: Can only accept Resource type STONE, not HOUSE.");
            }
        } else if (object == null) {
        } else {
            throw new IllegalArgumentException("Stone Block Factory: Can only accept objects of class type Resource.");
        }
    }
}
